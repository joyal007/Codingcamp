...
function CodeTitle({ currentTitle, setCurrentTitle }) {
    ...
    return (
        <div className='bg-gray-200 divide- h-12  text-gray-500 font-sans text-lg font-small   '>
            <span className='max-w-[40rem] h-full grid grid-cols-4 justify-start'>
                {
                    codeTitles.map((e) => {
                        return (
                            <div onClick={() => { setCurrentTitle(e.title) }} className={`cursor-pointer text-ellipse whitespace-pre max-w-[36rem]  px-3 flex overflow-hidden text-ellipsis justify-around items-center ${currentTitle === e.title ? 'bg-white' : ''}  h-full`}>
